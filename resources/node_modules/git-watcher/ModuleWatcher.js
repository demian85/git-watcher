...
ModuleWatcher.prototype._buildGitStatus = function(callback) {
	...
	function parse(gitFilesStatus) {
		async.each(gitFilesStatus, function(file, callback) {
			...
			var isFileDeleted = file.unstagedStatusStr === 'deleted' || file.stagedStatusStr === 'deleted';
			var addStatus = function(type, statusStr, diff, mimeType) {
				statuses[type].push({
			...
			var tasks = {};
			if (gitFileType === 'file' && !isFileDeleted) {
			...
			async.parallel(tasks, function(err, results) {
				if (err) return callback(err);
				if (isUnstaged) {
					addStatus('unstaged', file.unstagedStatusStr, results.unstaged, results.mimeType);
				}
				if (isStaged) {
					addStatus('staged', file.stagedStatusStr, results.staged, results.mimeType);
				}
				callback();
			});
		}, function(err) {
	...
	GitParser.getStatus(me.path, me.errEmitter.intercept(parse));
};
