...
ModuleWatcher.prototype._getFileDiff = function(filePath, isStaged, type, callback) {
	...
	var lineChunks = diff.map(function(change) {
		...
		/*var linesBefore = change.oldStart > 0 ? me._getRangeDiff(null, {
			start: oldMin - 3,
			end: oldMin,
			contents: oldLines
		}) : [];
		var linesAfter = change.newStart > 0 ? me._getRangeDiff(null, {
			start: newMin + change.newLines,
			end: newMin + change.newLines + 3,
			contents: oldLines
		}) : [];*/
		return me._getRangeDiff('-', oldRange).concat(me._getRangeDiff('+', newRange));
	});
	callback(null, lineChunks);
};

ModuleWatcher.prototype._getRangeDiff = function(type, range) {
	var lines = [];
