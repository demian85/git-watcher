...
ModuleWatcher.prototype.init = function() {
	console.log('Initializing module: ' + this.path);
...
ModuleWatcher.prototype._getFileDiff = function(filePath, isStaged, type, callback) {
	...
	var lineChunks = diff.map(function(change) {
		var oldMin = Math.max(change.oldStart-1, 0); // 0-based
		var newMin = Math.max(change.newStart-1, 0); // 0-based
		oldOffset += Math.max(change.newLines - change.oldLines, 0);
		var oldChunk = {
			start: oldMin,
			end: oldMin + change.oldLines,
			lines: change.oldLines,
			offset: oldOffset
		};
		var newChunk = {
			start: newMin,
			end: newMin + change.newLines,
	...
	var __log = relativeRepoFileName === 'LICENSE';
	var lines = me._buildLines(lineChunks, oldLines, newLines, __log);
...
ModuleWatcher.prototype._buildLines = function(lineChunks, oldLines, newLines, log) {
	var me = this;
	var lines = [], contextLines = 3000;
	...
	function addContextLines(start, end, offset) { // 0-based
		addLines(null, {
			start: start,
			end: end,
			offset: offset
	...
	function getRangeStart(chunk) {
		return Math.min(chunk.old.start + chunk.old.offset, chunk.new.start);
	}
	function getRangeEnd(chunk) {
		return Math.max(chunk.old.end, chunk.new.end);
	}
	
	if (lineChunks.length > 0) {
		//add(lineChunks[0].line - contextLines - 1, lineChunks[0].line - 1);
		for (var i = 0; i < lineChunks.length-1; i++) {
			addChunk(lineChunks[i]);
			var deletedOffset = Math.min(lineChunks[i].new.lines - lineChunks[i].old.lines, 0);
			var nextRangeStart = getRangeStart(lineChunks[i+1]);
			var thisRangeEnd = getRangeEnd(lineChunks[i]) + deletedOffset;
			var diff = nextRangeStart - thisRangeEnd;
			if (log) {
				console.log(thisRangeEnd, 'thisRangeEnd')
				console.log(nextRangeStart, 'nextRangeStart')
				console.log(deletedOffset, 'deletedOffset')
				console.log(diff, 'diff')
			}
			if (diff <= contextLines*2) {
				addContextLines(thisRangeEnd, thisRangeEnd + diff);
			} else {
				addContextLines(thisRangeEnd, thisRangeEnd + contextLines);
				addContextLines(nextRangeStart - contextLines, nextRangeStart);
			}
		}
		if (lineChunks.length > 1) {
			addChunk(lineChunks[lineChunks.length-1]);
			//add(lineChunks[lineChunks.length-1].line, lineChunks[lineChunks.length-1].line + contextLines);
		}
	}
