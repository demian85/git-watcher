...
ModuleWatcher.prototype._buildRanges = function(chunks, contents) {
	var me = this;
	var groups = [], 
		ranges = [],
	...
	if (chunks.length > 0) {
		add(chunks[0].line - contextLines - 1, chunks[0].line - 1);
		for (var i = 0; i < chunks.length-1; i++) {
			groups.push(chunks[i]);
			var lineOffset = chunks[i].line;
			var diff = chunks[i+1].line - lineOffset;
			if (diff <= contextLines*2) {
				add(lineOffset, lineOffset + diff - 1);
			} else {
				add(lineOffset, lineOffset + contextLines);
				add(chunks[i+1].line - contextLines - 1, chunks[i+1].line - 1);
			}
		}
		if (chunks.length > 1) {
			groups.push(chunks[chunks.length-1]);
			add(chunks[chunks.length-1].line, chunks[chunks.length-1].line + contextLines);
		}
