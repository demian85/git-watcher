...
ModuleWatcher.prototype._getFileDiff = function(filePath, isStaged, type, callback) {
	...
	diff.forEach(function(change) {
		...
		lineChunks = lineChunks.concat(me._getRangeLines('-', oldChunk)).concat(me._getRangeLines('+', newChunk));
	});
	callback(null, me._buildRanges(lineChunks, newLines));
};

ModuleWatcher.prototype._buildRanges = function(lineChunks, contents) {
	var me = this;
	var groups = [], 
		ranges = [],
		contextLines = config.diff.contextLines || 3;
	
	function add(start, end) { // 0-based
		groups = groups.concat(me._getRangeLines(null, {
			start: start,
			end: end,
			contents: contents
		}));
	...
	if (lineChunks.length > 0) {
		...
		for (var i = 0; i < lineChunks.length-1; i++) {
			groups.push(lineChunks[i]);
			var lineOffset = lineChunks[i].line;
			var diff = lineChunks[i+1].line - lineOffset;
			if (diff <= contextLines*2) {
				add(lineOffset, lineOffset + diff - 1);
			} else {
				add(lineOffset, lineOffset + contextLines);
				add(lineChunks[i+1].line - contextLines - 1, lineChunks[i+1].line - 1);
			}
		}
		if (lineChunks.length > 1) {
			groups.push(lineChunks[lineChunks.length-1]);
		...
		for (i = 0; i < groups.length; i++) {
			rangeLines.push(groups[i]);
			if (i === (groups.length - 1) || groups[i+1].line > groups[i].line + 1) {
