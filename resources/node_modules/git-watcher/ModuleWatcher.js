		...
		async.each(gitFilesStatus, function(file, callback) {
			...
			async.waterfall([
				function(callback) { // detect file mime type
					if (gitFileType !== 'file' || !fs.existsSync(filePath)) {
						return callback(null, null);
					}
					me.mmmagicInstance.detectFile(filePath, function(err, mimeType) {
						if (err) return callback(err);
						var isBinary = /charset=(binary|unknown)/i.test(mimeType);
						callback(null, {isBinary: isBinary, mimeType: mimeType});
					});
				},
