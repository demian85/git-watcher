...
var Git = {
	...
	fetch: function(modulePath, remote, upstreamRef, callback) {
		this._exec(modulePath, 'git fetch', [remote, upstreamRef], callback);
	},
	...
	checkoutBranch: function(modulePath, branch, callback) {
		this._exec(modulePath, 'git checkout', [branch], callback);
	},
	
	createBranch: function(modulePath, branchName, doCheckout, callback) {
		if (doCheckout) this._exec(modulePath, 'git checkout -b', [branchName], callback);
		else this._exec(modulePath, 'git branch', [branchName], callback);
	},
	...
	getLocalBranches: function(modulePath, callback) {
		this._exec(modulePath, 'git for-each-ref --sort=-committerdate --format="%(refname:short)\t%(upstream:short)\t%(authorname)\t%(authoremail)\t%(objectname:short)\t%(authordate:local)\t%(contents:subject)" refs/heads/', [], function(err, stdout) {
			if (err) return callback(err);
	...
	stashSave: function(modulePath, callback) {
		this._exec(modulePath, 'git stash', [], callback);
	},
	
	stashPop: function(modulePath, callback) {
		this._exec(modulePath, 'git stash pop', [], callback);
	},
