...
var Git = {
	...
	getLocalBranches: function(modulePath, callback) {
		this._exec(modulePath, 'git for-each-ref --sort=-committerdate --format="%(refname:short)\t%(upstream:short)" refs/heads/', [], function(err, stdout) {
			if (err) return callback(err);
			var branches = stdout.trim().split(/[\r\n]/).map(function(line) {
				var parts = line.split(/\t/);
				return {name: parts[0], upstream: parts[1] || ''};
			});
