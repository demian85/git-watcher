...
ModuleWatcher.prototype._buildLines = function(lineChunks, oldLines, newLines) {
	...
	if (lineChunks.length > 0) {
		for (var i = 0, len = lineChunks.length; i < len; i++) {
			...
			var oldNewDiff = Math.max(lineChunks[i].old.lines - lineChunks[i].new.lines, 0);
			...
			if (linesBetween <= contextLines*2) {
				...
				addContextLines(thisRangeStart + lineChunks[i].new.lines, thisRangeEnd - oldNewDiff + contextLines);
				addSeparator();
				if (i < (len - 1)) {
