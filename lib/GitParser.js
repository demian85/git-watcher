module.exports = {
	...
	getStatus: function(modulePath, callback) {
		var me = this;
		require('child_process').exec('git status --porcelain -z', {cwd: modulePath}, function execOutput(err, stdout, stderr) {
	...
	getSubmoduleSummary: function(modulePath, submoduleRelativePath, callback) {
		...
		require('child_process').exec(cmd, {cwd: modulePath}, function execOutput(err, stdout) {
			callback(err, stdout);
		});
	},
