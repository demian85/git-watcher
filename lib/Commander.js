...
function getLinePatchFromDiff(diff, line) {
	var regex = /(\n@@\s*-(\d+),\d+\s+\+(\d+),\d+\s*@@.*?\n)([\s\S]+)/g;
	var patch = getPatchFromDiff(diff, line);
	return patch.replace(regex, function(m0, m1, deletionStart, additionStart, lineContents) {
		deletionStart = parseInt(deletionStart);
		additionStart = parseInt(additionStart);
		lineContents = lineContents.split(/\n/);
		var deletedLinesMap = {};
		var deletedOffset = additionStart - deletionStart;
		if (line.type === '-') {
			...
			lineContents = lineContents.filter(function(lineContent) {
				var isDeleted = /^-/.test(lineContent);
				if (isDeleted) deletedLinesMap[deletionStart++] = lineContent.replace(/^-/, ' ');
				return !isDeleted;
			}).map(function(lineContent) {
				console.log(deletedLinesMap)
				var content;
				if (line.number === additionStart) {
					content = lineContent;
				} else {
					content = deletedLinesMap[additionStart - deletedOffset] || lineContent.replace(/^\+/, ' ');
				}
				additionStart++;
				return content;
			});
		}
		return m1 + lineContents.join('\n');
	});
}
...
Commander.prototype.stageLine = function(file, line, callback) {
	var me = this;
	this._exec('git diff', [file.name], function(err, output) {
		if (err) return callback(err);
		var patch = getLinePatchFromDiff(output, line);
		require('fs').writeFileSync(me.modulePath + '/patch.diff', patch);
...
Commander.prototype.unstageLine = function(file, line, callback) {
	var me = this;
	this._exec('git diff', ['--cached', '-R', file.name], function(err, output) {
		if (err) return callback(err);
		var patch = getLinePatchFromDiff(output, line);
		me._applyPatch(patch, true, callback);
	});
...
Commander.prototype.checkoutTheirs = function(file, callback) {
	this._execEnd('git checkout --theirs --', [file.name], callback);
};

Commander.prototype.checkoutOurs = function(file, callback) {
	this._execEnd('git checkout --ours --', [file.name], callback);
};
