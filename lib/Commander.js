...
Commander.prototype.getFileTopCommitters = function(file, limit, callback) {
	this._execEnd('git log --pretty=short | git --no-pager shortlog -sne --', [file.name], function(err, output) {
		if (err) return callback(err);
